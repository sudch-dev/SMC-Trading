...
# ---------- NEW: Moneyness controls ----------
ATM_BAND_PCT = float(os.getenv("ATM_BAND_PCT", "0.0025"))  # ±0.25% of spot treated as ATM
EXCLUDE_ATM_NEAR = os.getenv("EXCLUDE_ATM_NEAR", "1") in ("1","true","True")  # block ATM shorts when dte ≤ NEAR_DAYS
...
# ---------- NEW: Moneyness (uses UNDERLYING LTP vs STRIKE) ----------
def _classify_moneyness(spot, strike, side):
    """
    spot: underlying LTP
    strike: option strike
    side: 'CE' or 'PE' (used only for ITM/OTM direction)
    """
    try:
        spot = float(spot); strike = float(strike)
    except Exception:
        return "NA"
    if spot <= 0 or strike <= 0: return "NA"
...
def _apply_greeks_gates(side, base_trade_type, ltp, iv, greeks, days, params, moneyness):
    """
    NOTE: 'moneyness' is computed from UNDERLYING spot vs strike (not option LTP).
    We also optionally block ATM shorts when dte ≤ NEAR_DAYS.
    """
    if iv is None or greeks.get("delta") is None:
        if ALLOW_IF_NO_GREEKS:
            note = f"Greeks missing (allowed) dte={days}"
            # If near-expiry and ATM and short, still warn/soft-block
            if base_trade_type == "SHORT" and EXCLUDE_ATM_NEAR and days <= NEAR_DAYS and moneyness == "ATM":
                return "SHORT", note + " | ATM near-expiry (soft block)", False
            return base_trade_type, note, True
    ...
    # NEW: explicit ATM block near expiry (uses spot vs strike)
    if EXCLUDE_ATM_NEAR and days <= NEAR_DAYS and moneyness == "ATM":
        fails.append("ATM near-expiry (block)")
...
def run_smc_scan(kite):
    ...
    try:
        ...
        # ---------- NEW: quote UNDERLYINGS for live spot LTP ----------
        underliers = sorted({nm for *_, nm in candidates})
        spot_quotes = {}
        if underliers:
            # Kite expects exchange prefix
            syms = [f"NSE:{nm}" for nm in underliers]
            for i in range(0, len(syms), 100):
                batch = syms[i:i+100]
                try:
                    spot_quotes.update(kite.quote(batch) or {})
                except Exception as e:
                    out["errors"].append(f"underlying quote error: {str(e)}")

        def _spot_ltp(nm):
            q = spot_quotes.get(f"NSE:{nm}") or {}
            lp = q.get("last_price")
            return float(lp) if lp else float(last_px.get(nm) or 0.0)  # fallback to last close
        ...
        for sym, meta, side, base_type, rationale, nm in candidates:
            ...
            # IV + Greeks inputs
            S=_spot_ltp(nm)                      # <-- LIVE underlying LTP (fallback to last close)
            ...
            # NEW: moneyness via spot vs strike
            moneyness = _classify_moneyness(S, K, side)
            ...
            # Dynamic gates based on dte (now includes moneyness awareness)
            params=_dynamic_params(dte)
            final_type, greek_note, _passed = _apply_greeks_gates(side, base_type, ltp, iv, gks, dte, params, moneyness)
            ...
            scored.append({
                ...
                "name": nm,
                "type": side,
                "trade_type": final_type,     # LONG/SHORT after dynamic Greeks + moneyness gates
                ...
                "spot": float(S),             # <-- NEW: include underlying spot used
                "ltp": float(ltp),
                "lot_size": int(lot),
                "moneyness": moneyness,       # <-- NEW
