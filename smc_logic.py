...
# ---------- Config ----------
BUDGET = float(os.getenv("BUDGET", "1000"))          # total ₹ budget (split across 5 ideas)
RING_STRIKES = int(os.getenv("RING_STRIKES", "4"))   # strike steps on each side of ATM
UNDERLYINGS = ("NIFTY", "BANKNIFTY")                 # focus universe
DEBUG_SCAN = os.getenv("DEBUG_SCAN", "0") in ("1", "true", "True")

# module-level cache to avoid huge downloads on every hit
...
def _ring(strikes, atm, steps):
    ...
    lo = max(0, i - steps)
    hi = min(len(strikes) - 1, i + steps)
    return set(strikes[lo:hi + 1])

def _score(q, lot, ltp):
    # prefer liquidity + tight spread + affordability
...
def run_smc_scan(kite):
    ...
    Direction from underlying (daily):
      Bullish if EMA5>EMA10 & Price>R1 & RSI<70 -> prefer CE
      Bearish if EMA5<EMA10 & Price<S1 & RSI>30 -> prefer PE
      Else -> BOTH sides allowed.
    Returns up to 5 picks, each with keys used by the frontend.
    """
    out = {
        "status": "ok",
    ...
    try:
        ...
        if not base:
            out["status"] = "error"
            out["errors"].append("No NFO rows for NIFTY/BANKNIFTY")
            return out
        ...
        # 4) candidates near ATM respecting bias
        candidates = []
        for nm in UNDERLYINGS:
            ...
            for r in rows:
                if r.get("strike") in ring and r.get("instrument_type") in allowed:
                    ...
                    candidates.append((sym, r, r.get("instrument_type")))
        ...
        if not out["picks"]:
            out["status"] = "degraded"
            out["errors"].append("Quotes empty (off-hours?)")
